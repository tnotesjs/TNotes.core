/**
 * Shared TNotes Markdown configuration used by the independent SSG and the
 * frozen VitePress adapter.
 *
 * Markdown 配置
 */
...
export interface TNotesMarkdownOptions {
  lineNumbers?: boolean;
  math?: boolean;
  config?: (md: MarkdownIt) => void;
  anchor?: { slugify?: (value: string) => string };
  image?: { lazyLoading?: boolean };
}
 ...
 * Markdown 配置
 */
export function getTNotesMarkdownConfig(): TNotesMarkdownOptions {
  const markdown: TNotesMarkdownOptions = {
    lineNumbers: true,
    math: true,
    config(md) {
      // 添加前置规则保存原始内容
